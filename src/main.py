...
def pep(session):
    ...
    if response is None:
        return
    soup = BeautifulSoup(response.text, features='lxml')
    main_tag = find_tag(soup, 'section', attrs={'id': 'index-by-category'})
    pep_link = main_tag.find_all('tr')
    to_count = []
    results = [('Статус', 'Количество')]
    for link in tqdm(pep_link):
        pep_a_tag = link.find('a')
        pep_status = link.find('abbr')
        if pep_a_tag is None:
            continue
        href = pep_a_tag['href']
        pep_doc_link = urljoin(PEP_DOC_URL, href)
        preview_status = pep_status.text[1:]

        response = get_response(session, pep_doc_link)
        if response is None:
            continue
        soup = BeautifulSoup(response.text, 'lxml')
        main_info = find_tag(soup, 'dl',
                             {'class': 'rfc2822 field-list simple'})
        dt_tags = main_info.find_all('dt')
        for dt in dt_tags:
